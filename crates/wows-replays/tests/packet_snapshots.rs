...
fn snapshot_first_n_packets(replay_filename: &str, n: usize) {
    ...
    let snapshot_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests").join("snapshots").join(replay_stem);
    ...
    settings.bind(|| {
        while !remaining.is_empty() && i < n {
            ...
            let value = serde_json::to_value(&decoded).expect("should serialize decoded packet");
...
fn snapshot_vermont_packets() {
    snapshot_first_n_packets("20260213_143518_PASB110-Vermont_22_tierra_del_fuego.wowsreplay", 50);
}
...
fn snapshot_marceau_packets() {
    snapshot_first_n_packets("20260213_203056_PFSD210-Marceau_22_tierra_del_fuego.wowsreplay", 50);
}
...
fn snapshot_narai_pve_packets() {
    snapshot_first_n_packets("20260223_115252_PZSC718-Narai_s06_Atoll.wowsreplay", 50);
}
...
fn snapshot_hull_packets() {
    snapshot_first_n_packets("20250206_020938_PASD720-Hull_47_Sleeping_Giant.wowsreplay", 50);
}
...
fn snapshot_s189_submarine_packets() {
    snapshot_first_n_packets("20230419_203306_PRSS508-S-189_42_Neighbors.wowsreplay", 50);
}

...
//!
//! These live here (rather than in wowsunpack) because they need wows-data-mgr
//! to locate a game install; keeping them out of wowsunpack avoids a
//! wowsunpack -> wows-data-mgr dev-dependency cycle. They exercise wowsunpack's
//! VFS over a wows-data-mgr-resolved build and skip at runtime when no game
//! data is available.
...
fn vfs_file_tree_has_expected_directories() {
    let Some((build, vfs)) = wows_data_mgr::latest_build() else {
...
fn vfs_can_traverse_subdirectories() {
    let Some((_build, vfs)) = wows_data_mgr::latest_build() else {
        eprintln!("Skipping: no game data available");
        return;
    };

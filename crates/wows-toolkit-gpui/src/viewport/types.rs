...
/// 2D vector type used for screen-space math. Alias for `nalgebra::Vector2<f32>`.
pub type Vec2 = na::Vector2<f32>;

/// A plain axis-aligned viewport rectangle in screen space (top-left origin,
/// Y-down), replacing `egui::Rect` at the module boundary. `x`/`y` is the
...
impl ViewRect {
    ...
    pub fn from_min_size(min: (f32, f32), size: (f32, f32)) -> Self {
        Self { x: min.0, y: min.1, w: size.0, h: size.1 }
    }
    ...
    /// Whether a screen point lies inside the rect.

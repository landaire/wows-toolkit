    ...
    modifiers
        ...
        .filter_map(|(modifier_name, modifier_data)| {
            ...
            if matches!(
                ...
                "excludedConsumables" | "fireResistanceEnabled" | "priorityTargetEnabled" | "artilleryAlertEnabled" | "nearEnemyIntuitionEnabled" | "GMRotationSpeed"
            ) {
                return None;
            }
            let modifier = if let Some(common_value) = modifier_data.i64_ref().cloned() {
                ...
                let modifier_data = modifier_data.dict_ref().expect("skill modifier data is not a dict");
...
fn build_crew_skills(skills: &BTreeMap<HashableValue, Value>) -> Result<Vec<CrewSkill>, CrewSkillBuilderError> {
    skills
        .iter()
        .map(|(skill_name, skill_data)| {
            let skill_name = skill_name.string_ref().expect("skill_name is not a String").to_owned();
            ...
            let logic_trigger = CrewSkillLogicTriggerBuilder::default()
                ...
                .divider_type(game_param_to_type!(logic_trigger_data, "dividerType", Option<String>))
                .divider_value(game_param_to_type!(logic_trigger_data, "dividerValue", Option<f32>))
                ...
                .flood_count(game_param_to_type!(logic_trigger_data, "floodCount", Option<usize>))
                .health_factor(game_param_to_type!(logic_trigger_data, "healthFactor", Option<f32>))
...
fn build_crew_personality(personality: &BTreeMap<HashableValue, Value>) -> Result<CrewPersonality, CrewPersonalityBuilderError> {
    ...
    let ships = CrewPersonalityShipsBuilder::default()
        .groups(
            game_param_to_type!(ships, "groups", &[()])
                .iter()
                .map(|value| value.string_ref().expect("group entry is not a string").to_owned())
                .collect(),
        )
        .nation(
            game_param_to_type!(ships, "nation", &[()])
                .iter()
                .map(|value| value.string_ref().expect("nation entry is not a string").to_owned())
                .collect(),
        )
        .peculiarity(
            game_param_to_type!(ships, "peculiarity", &[()])
                .iter()
                .map(|value| value.string_ref().expect("peculiarity entry is not a string").to_owned())
                .collect(),
        )
        .ships(
            game_param_to_type!(ships, "ships", &[()])
                .iter()
                .map(|value| value.string_ref().expect("ships entry is not a string").to_owned())
                .collect(),
        )
        .build()
...
fn build_ability_category(category_data: &BTreeMap<HashableValue, Value>) -> Result<AbilityCategory, AbilityCategoryBuilderError> {
    let reload_time = if let Some(reload_time) = category_data.get(&HashableValue::String("reloadTime".to_owned())) {
    ...
    let work_time = if let Some(work_time) = category_data.get(&HashableValue::String("workTime".to_owned())) {
...
fn build_ability(ability_data: &BTreeMap<HashableValue, Value>) -> Result<Ability, AbilityBuilderError> {
    let test_key = HashableValue::String("numConsumables".to_string());
    let categories: HashMap<_, _> = HashMap::from_iter(ability_data.iter().filter_map(|(key, value)| {
...
fn build_ship(ship_data: &BTreeMap<HashableValue, Value>) -> Result<Vehicle, VehicleBuilderError> {
        ...
        .map(|(slot_name, slot_data)| {
            ...
            let abils: Vec<(String, String)> = abils
                ...
                .map(|abil| {
                    ...
                    (
                        abil[0].string_ref().expect("abil[0] is not a string").clone(),
                        abil[1].string_ref().expect("abil[1] is not a string").clone(),
                    )
                })
                .collect();
    ...
    VehicleBuilder::default().level(level).group(group).abilities(abilities).build()
}
...
impl GameMetadataProvider {
    pub fn from_pkg(file_tree: &FileNode, pkg_loader: &PkgFileLoader, game_version: usize) -> Result<GameMetadataProvider, DataLoadError> {
        ...
        let params = if let Some(params) = params {
            ...
            let params_dict = params.dict_ref().expect("First element of GameParams is not a dictionary");
